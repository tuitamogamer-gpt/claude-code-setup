...
def summarize_csv(file_path, output_dir='.'):
    """
    ...
    Charts are saved as PNG files into `output_dir` (default: current dir).
    """
    import os
    ...
    if numeric_cols:
        ...
        if len(numeric_cols) > 1:
            ...
            plt.tight_layout()
            plt.savefig(_chart('correlation_heatmap.png'), dpi=150)
            plt.close()
            charts_created.append(_chart('correlation_heatmap.png'))
    
    # Categorical analysis
    categorical_cols = df.select_dtypes(exclude=['number', 'datetime', 'datetimetz']).columns.tolist()
    ...
    if date_cols:
        ...
        if numeric_cols:
            ...
            plt.tight_layout()
            plt.savefig(_chart('time_series_analysis.png'), dpi=150)
            plt.close()
            charts_created.append(_chart('time_series_analysis.png'))
    ...
    if numeric_cols:
        ...
        plt.tight_layout()
        plt.savefig(_chart('distributions.png'), dpi=150)
        plt.close()
        charts_created.append(_chart('distributions.png'))
    ...
    if categorical_cols:
        ...
        plt.tight_layout()
        plt.savefig(_chart('categorical_distributions.png'), dpi=150)
        plt.close()
        charts_created.append(_chart('categorical_distributions.png'))
...
if __name__ == "__main__":
    import sys
    file_path = sys.argv[1] if len(sys.argv) > 1 else "resources/sample.csv"
    outdir = sys.argv[2] if len(sys.argv) > 2 else "."
    print(summarize_csv(file_path, output_dir=outdir))
