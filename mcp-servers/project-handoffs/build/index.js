...
class ProjectManager {
    ...
    validateTemplate(type, data) {
        ...
        if (!template)
            return; // Non-templated types are valid
        ...
        if (missingFields.length > 0) {
            throw new ProjectError(`Missing required fields for ${type}: ${missingFields.map(f => f.name).join(', ')}`, data.projectId || 'validation');
        }
    }
    ...
    async createNextStep(projectId, step) {
        ...
        if (step.dependencies?.length) {
            const missingDeps = step.dependencies.filter(depId => !data.nextSteps.some(s => s.id === depId));
    ...
    async startWorkingSession(projectId, nextStepId) {
        const data = await this.loadProjectData(projectId);
    ...
    async createHandoff(projectId, sessionId, handoff) {
    ...
    async getLatestNextSteps(projectId) {
        const data = await this.loadProjectData(projectId);
    ...
    async getNextStepHistory(projectId, stepId) {
        const data = await this.loadProjectData(projectId);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "delete_project":
                await projectManager.deleteProject(args.projectId);
                return {
                    content: [{
                            type: "text",
            ...
            case "create_next_step":
                const step = await projectManager.createNextStep(args.projectId, {
            ...
            case "start_working_session":
                const session = await projectManager.startWorkingSession(args.projectId, args.nextStepId);
                return {
                    content: [{
                            type: "text",
            ...
            case "create_handoff":
                const handoff = await projectManager.createHandoff(args.projectId, args.sessionId, {
                    completedWork: args.completedWork,
                    codeState: args.codeState,
                    environmentState: args.environmentState,
                    unresolvedIssues: args.unresolvedIssues || [],
                    newNextSteps: args.newNextSteps || []
                });
                return {
                    content: [{
                            type: "text",
            ...
            case "get_latest_next_steps":
                const steps = await projectManager.getLatestNextSteps(args.projectId);
                return {
                    content: [{
                            type: "text",
            ...
            case "get_next_step_history":
                const history = await projectManager.getNextStepHistory(args.projectId, args.stepId);
                return {
                    content: [{
                            type: "text",
