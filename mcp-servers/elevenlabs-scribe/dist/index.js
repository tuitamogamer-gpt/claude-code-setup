...
import { EXT_BY_FORMAT, buildStructured, defaultDir, renderBody, resolvePath, saveExportedFormats, sourceBaseName, uniquePath, } from "./core.js";
...
import { BatchTranscribeInputSchema, TranscribeInputSchema, } from "./schemas.js";
...
function applyScript(resp, script) {
...
server.registerTool("elevenlabs_transcribe_audio", {
  ...
  - "Validation error ..." (HTTP 422) for bad parameters or unsupported files.`,
    ...
    try {
        if (!args.file_path && !args.source_url) {
            return errorResult("Error: Provide either 'file_path' (a local file) or 'source_url' (a remote URL).");
        }
        if (args.file_path && args.source_url) {
        ...
        if (wantsExports) {
            ...
            if (exportedFiles.length)
                structured.exported_files = exportedFiles;
        }
...
server.registerTool("elevenlabs_transcribe_batch", {
...
Examples:
  ...
  - "Transcribe these clips and save docx for each" -> { file_paths: [...], export_formats: ["docx"] }`,
    ...
    try {
        const items = [
            ...(args.file_paths ?? []).map((value) => ({ kind: "file", value })),
            ...(args.source_urls ?? []).map((value) => ({ kind: "url", value })),
        ];
        if (items.length === 0) {
            return errorResult("Error: Provide at least one entry in 'file_paths' or 'source_urls'.");
        }
        const wantsExports = (args.export_formats?.length ?? 0) > 0;
        const diarize = args.diarize || wantsExports;
        let granularity = args.timestamps_granularity;
        ...
        for (const r of results) {
            if (r.ok) {
                ...
                lines.push(`- ✅ **${r.input}** — ${r.language_code ?? "?"}, ${dur}, ${r.word_count} words → \`${r.saved_to}\``);
...
server.registerTool("elevenlabs_check_connection", {
...
Returns: human-readable status text. Reports "Missing ELEVENLABS_API_KEY" or an authorization error if the key is absent or invalid.`,
    ...
    try {
        ...
        if (sub.character_count != null && sub.character_limit != null) {
            lines.push(`- **TTS characters used:** ${sub.character_count.toLocaleString()} / ${sub.character_limit.toLocaleString()}`);
        }
...
async function main() {
    ...
    if (!process.env.ELEVENLABS_API_KEY) {
        console.error(`[${SERVER_NAME}] WARNING: ELEVENLABS_API_KEY is not set. Tools will return a configuration error until it is provided.`);
    }
