...
const LOWER_DIGRAPH: Record<string, string> = { љ: "lj", њ: "nj", џ: "dž" };

// Uppercase digraphs map to [ALL-CAPS, Title-case] depending on the next letter.
const UPPER_DIGRAPH: Record<string, [string, string]> = {
  Љ: ["LJ", "Lj"],
  Њ: ["NJ", "Nj"],
  Џ: ["DŽ", "Dž"],
};
...
export function transliterateResponseInPlace(resp: TranscriptionResponse): void {
  if (resp.text) resp.text = serbianCyrillicToLatin(resp.text);
  for (const word of resp.words ?? []) {
    if (word.text) word.text = serbianCyrillicToLatin(word.text);
    if (word.characters) {
      for (const c of word.characters) {
        if (c.text) c.text = serbianCyrillicToLatin(c.text);
      }
    }
  }
