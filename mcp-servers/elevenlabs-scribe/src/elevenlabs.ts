...
export function getApiKey(): string {
  ...
  if (!key) {
    throw new ElevenLabsError(
      "Missing ELEVENLABS_API_KEY. Set it in your MCP client configuration (the server's env) before transcribing. " +
...
function extractApiMessage(detail: unknown): string | undefined {
  if (!detail || typeof detail !== "object") {
    return typeof detail === "string" && detail ? detail : undefined;
  }
  const inner = (detail as Record<string, unknown>).detail ?? detail;
  if (typeof inner === "string") return inner;
  if (Array.isArray(inner)) {
    const msgs = inner
      .map((e) =>
        e && typeof e === "object"
          ? ((e as Record<string, unknown>).msg ?? (e as Record<string, unknown>).message)
          : undefined,
      )
...
async function buildHttpError(res: Response): Promise<ElevenLabsError> {
  let bodyText = "";
  let detail: unknown;
  try {
...
export async function transcribeAudio(params: TranscribeParams): Promise<TranscriptionResponse> {
  ...
  if (params.filePath) {
    ...
    if (!info || !info.isFile()) {
      throw new ElevenLabsError(
        `File not found or not a regular file: ${params.filePath}. Provide a valid path to an audio/video file.`,
      );
    }
    if (info.size === 0) {
    ...
    if (info.size > MAX_FILE_BYTES) {
      throw new ElevenLabsError(
        `File is ${(info.size / 1e9).toFixed(2)} GB, which exceeds the 5 GB direct-upload limit. ` +
  ...
  if (params.exportFormats && params.exportFormats.length > 0) {
    ...
    form.append(
      "additional_formats",
      JSON.stringify(params.exportFormats.map((format) => ({ format }))),
    );
  }
...
export async function checkConnection(): Promise<UserInfo> {
  const apiKey = getApiKey();
  let res: Response;
  try {
